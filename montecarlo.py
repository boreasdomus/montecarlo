...
def diagnose_garch(std_resid, alpha, gamma, beta):
    ...
    lb_result = acorr_ljungbox(std_resid**2, lags=[20], return_df=True)
...
def calc_statistics(prices, start_price, target_price, num_paths, stopped, stop_day, stop_type, short=False):
    ...
    median_win_pnl = float(np.median(pnl[win_mask])) if num_wins > 0 else 0.0
    median_loss_pnl = float(np.median(pnl[loss_mask])) if num_losses > 0 else 0.0

    payoff_ratio = None
    be_win_rate = 0.0

    if num_losses > 0 and median_loss_pnl != 0:
        payoff_ratio = float(abs(median_win_pnl / median_loss_pnl))
    ...
    if payoff_ratio and payoff_ratio > 0 and median_win_pnl > 0:
...
def save_report(ticker, stock_name, last_close, start_price, target_price,
                sim_days, num_paths, stop_price_fixed, stop_loss_pct, stop_loss_label,
                trailing_stop, trailing_stop_pct, trailing_label, stats, diagnostics, fit_window, lookback_used, short=False):
    ...
    with open(output_file, "w", encoding="utf-8") as f:
        ...
        f.write(f"Stop-Loss: {stop_price_fixed:.2f} ({stop_loss_label})\n")
        f.write(f"Trailing Stop: {'Yes (' + trailing_label + ')' if trailing_stop else 'No'}\n")
...
def main():
    ...
    def parse_stop(value, atr_pct, start_price, short, kind):
        ...
        Returnerar (pct, label) där label beskriver hur värdet tolkades."""
        s = str(value).strip().lower()
        if s.endswith("xatr"):
            if atr_pct is None:
                print(f"  Varning: ATR saknas (OHLC?) — {kind} faller tillbaka till 3%")
                return 0.03, "3.0% (ATR saknas)"
            mult = float(s[:-4]) if s[:-4] else 1.0
            pct = mult * atr_pct
            return pct, f"{mult:g}×ATR ≈ {pct:.1%}"
        val = float(s)
        if val > 1:  # Absolute price
            ...
            pct = max(pct, 0.001)
            return pct, f"{pct:.1%} → {val:g}"
        return val, f"{val:.1%}"

    stop_loss_pct, stop_loss_label = parse_stop(args.stop_loss, atr_pct, start_price, short, "stop-loss")
    trailing_raw, trailing_label = parse_stop(args.trailing_stop, atr_pct, start_price, short, "trailing")
    ...
    output_file = save_report(
        ...
        sim_days, num_paths, stop_price_fixed, stop_loss_pct, stop_loss_label,
        trailing_stop, trailing_stop_pct, trailing_label, stats, diagnostics,
